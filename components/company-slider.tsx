        ...
        {[...companies, ...companies.slice(0, 4)].map((company, index) => (
          <div key={index} className="px-4">
            <div className="animate-slidein">
              <img src={company.logo} alt={company.name} className="mx-auto" style={{ maxWidth: '100%', maxHeight: '100px' }} />
              <p className={`text-center mt-2 ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'}`}>{company.name}</p>
            </div>
          </div>
        ))}
      </div>
