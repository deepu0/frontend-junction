...
const LandingPage: React.FC = () => {
  ...
  const interviewData = {
    title: "Google Frontend Developer Interview",
    imageSrc: "https://via.placeholder.com/150",
    description: "Prepare with Confidence: Gain insights into the interview process, types of questions asked, and the overall experience shared by fellow developers who've been through it.",
    tags: ["Google", "Frontend"]
  };
  ...
  return (
    <div className={`flex flex-col min-h-screen `}>
      <div className="flex flex-col gap-8 container mx-auto mt-14 px-4 flex-grow">
        ...
        <p className={`relative font-regular text-sm sm:text-xl text-zinc-500 tracking-wide mb-8 text-center max-w-2xl antialiased  ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'} mx-auto`}>Prepare with Confidence: End your interview prep frustration. Get frontend interview experiences from across the web, all in one place.</p>
        <div className="flex flex-row md:flex-row space-y-4 md:space-y-0 space-x-0 md:space-x-4 mt-10 gap-2">
        <button className="w-40 h-10 rounded-xl bg-black border dark:border-white border-transparent text-white text-sm">
          Join now
        </button>
        <button className="w-40 h-10 rounded-xl bg-white text-black border border-black  text-sm mt-0">
          Signup
        </button>
      </div>
        {/* Card Component */}
        ...
        <div className={`text-center mt-8 ${theme === 'light' ? 'text-gray-700' : 'text-gray-300'}`}>
