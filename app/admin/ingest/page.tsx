...
export default function IngestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!url || !content) {
      setStatus({ msg: 'URL and content are required', type: 'error' });
      return;
    }
    if (content.length < 200) {
      setStatus({ msg: 'Content too short (min 200 chars)', type: 'error' });
      return;
    }
    ...
    try {
      const res = await fetch('/api/pipeline/ingest', {
        ...
        headers: {
          ...
          'x-capture-key': process.env.NEXT_PUBLIC_CAPTURE_SECRET || '',
        },
        body: JSON.stringify({
          url,
    ...
    <div className='min-h-screen bg-background pt-24 pb-16'>
      <div className='container mx-auto px-4 max-w-2xl'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          {status && (
            <div
              className={`p-3 rounded-lg text-sm ${status.type === 'success' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}
            >
              {status.msg}
            </div>
          )}
        ...
        <p className='text-xs text-muted-foreground mt-4 text-center'>
          Queued → AI processes every 6hrs → Score 8+ auto-publishes, 7 goes to
          review, below 7 rejected.
        </p>
      </div>
    </div>
  );
