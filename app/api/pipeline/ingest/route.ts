...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!url || !content)
      return NextResponse.json(
        { error: 'url and content required' },
    ...
    if (cleanContent.length < 200)
      return NextResponse.json(
        { error: 'Content too short (min 200 chars)' },
        { status: 400, headers: corsHeaders }
      );
    ...
    const { error } = await supabase.from('captured_content').upsert(
      {
    ...
    if (error) {
      if (error.code === '23505')
        return NextResponse.json(
          { success: true, message: 'Already captured' },
          { headers: corsHeaders }
        );
      return NextResponse.json(
        { error: error.message },
    ...
    return NextResponse.json(
      { success: true, message: 'Captured! Will be processed soon.' },
      { headers: corsHeaders }
    );
  } catch (err: any) {
    return NextResponse.json(
